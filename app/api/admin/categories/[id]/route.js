// Admin Category — update / enable-disable / delete.
...
export async function PATCH(req, { params }) {
  ...
  if (d.slug && await Category.exists({ slug: d.slug, _id: { $ne: id } })) return NextResponse.json({ ok: false, errors: { slug: ["Slug already in use"] } }, { status: 409 });
  const cat = await Category.findByIdAndUpdate(id, { $set: d }, { new: true });
