...
import { canAccess, resolveRole, isDemoMode, meetsRole } from "@/lib/auth";
...
export async function requireRole(minRole, { redirectTo } = {}) {
  ...
  if (!meetsRole(user.role, minRole)) redirect("/");
  return user;
}
...
export async function apiGuard(minRole) {
  ...
  if (!meetsRole(user.role, minRole)) return { ok: false, status: 403 };
  return { ok: true, user };
}
