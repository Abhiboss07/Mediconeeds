...
export async function issueOtp({ identifier, channel, purpose }) {
  ...
  if (recent >= MAX_PER_HOUR) { console.warn("[OTP-RATELIMIT] hit — not sending"); return { ok: false, error: "Too many requests. Try again later.", retryAfter: 3600 }; }
  ...
  console.log("[OTP-5] code generated & saved to Mongo");

  const mins = Math.round(TTL / 60);
  console.log(`[OTP-6] calling ${channel} provider…`);
  const delivery =
    channel === "email"
      ? sendEmail({
          to: id,
          subject: `Your Mediconeeds verification code`,
