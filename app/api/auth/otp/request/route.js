...
export async function POST(req) {
  ...
  // Unified passwordless email/SMS: a "login" for an unknown account is treated as
  // a sign-up — we still send a code and the account is created on verify. This
  // keeps behaviour identical whether or not the account exists (anti-enumeration)
  // AND avoids the confusing silent "code sent" with no email.
  // Reset is the exception: there's no password to reset for a missing account,
  // so we stay silent (still no enumeration leak).
  if (purpose === "reset" && !exists) {
    return NextResponse.json({ ok: true });
  }
  // For an explicit signup on an existing account, nudge them to log in.
