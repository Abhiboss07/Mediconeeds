...
/**
 * Real per-category counts for the storefront, computed from live published
 * CatalogProducts. Only categories that actually have products are returned, so
 * the home "Shop by Category" grid never shows an empty/fake category. The
 * `name` is what the PLP category filter matches on; `slug` drives the icon.
...
export async function getCategoryCounts() {
  ...
  for (const p of products) {
    const name = (p.categoryName || p.category || "").trim();
    if (!name) continue;
    const key = name.toLowerCase();
    if (!map.has(key)) map.set(key, { name, slug: (p.category || name).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, ""), count: 0, image: p.image || "", color: p.color || "#3056D3" });
    map.get(key).count += 1;
  }
