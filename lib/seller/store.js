...
// Hydrate the REAL logged-in seller's identity (company/status/avatar) so the
// header never shows the seeded demo seller. Merges over the seed so any other
// seed-only fields the UI relies on stay intact.
export async function hydrateSeller(force = false) {
  if (sellerHydrated && !force) return;
  sellerHydrated = true;
  try {
    const r = await (await fetch("/api/seller/profile")).json();
    if (r.ok && r.seller) { state.seller = { ...state.seller, ...r.seller }; emit(); }
  } catch { /* keep seeded identity */ }
}
...
export function useHydrateSeller() {
  useEffect(() => { hydrateSeller(); hydrateProducts(); hydrateOrders(); hydrateNotifications(); }, []);
}
