...
// The homepage category taxonomy (mirrors the reference design) — every card is
// always shown; the count is the REAL live number, or "Coming Soon" (count 0).
export const HOME_CATEGORIES = [
  { name: "Sunscreen", color: "#e0633a" }, { name: "Serum", color: "#8b1a89" },
  { name: "Cleanser", color: "#1e7a5a" }, { name: "Moisturiser", color: "#2f74d0" },
  { name: "Face Cream", color: "#d6336c" }, { name: "Hair Care", color: "#7c3aed" },
  { name: "Acne Care", color: "#e0633a" }, { name: "Anti-Ageing", color: "#7c3aed" },
  { name: "Pigmentation", color: "#1e7a5a" }, { name: "Lip Care", color: "#2f74d0" },
  { name: "Body Care", color: "#8a5a2b" }, { name: "Under Eye", color: "#2f74d0" },
];

/**
 * Merge the fixed home taxonomy with REAL live counts (fold-matched so
 * "Serum"↔"Serums"). Returns every taxonomy category, with `count` from the DB
 * (0 → the UI shows "Coming Soon"). No card is hidden.
 */
export async function getHomeCategoryCounts() {
  const db = await getCategoryCounts();
  const byKey = new Map(db.map((c) => [foldKey(c.name), c]));
  return HOME_CATEGORIES.map((t) => ({ name: t.name, color: t.color, count: byKey.get(foldKey(t.name))?.count || 0 }));
}
