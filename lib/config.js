...
// Explicit opt-in: true ONLY when the var is set to a truthy value. Unset OR
// empty OR "false"/"0" all resolve to false (secure default).
const optIn = (v) => v != null && v !== "" && v !== "false" && v !== "0";
// PRODUCTION KILL-SWITCH: demo mode is physically impossible when
// NODE_ENV=production, regardless of DEMO_MODE/DEMO_ROLE. A prod deploy can
// therefore never auto-authenticate a visitor. Mirrored in lib/auth.js.
const demoAllowed = (v) => process.env.NODE_ENV !== "production" && optIn(v);
...
export const config = {
  ...
  // Demo rollout. SECURE BY DEFAULT: demo mode is OFF unless DEMO_MODE is
  // explicitly set truthy. A fresh deploy with the var unset therefore enforces
  // real auth/RBAC — it must NEVER auto-authenticate a visitor as a seller.
  demo: { enabled: demoAllowed(process.env.DEMO_MODE), role: process.env.DEMO_ROLE || "seller" },

  // Email over SMTP (Nodemailer) — used for local dev. Many hosts (Render) block
  // outbound SMTP ports, so production sends via the Brevo HTTP API below.
  smtp: {
  ...
  // Email over HTTP (Brevo) — port 443, so it works where SMTP egress is blocked.
  // Preferred when BREVO_API_KEY is set. Sender falls back to SMTP_FROM.
  brevo: {
    apiKey: process.env.BREVO_API_KEY || "",
    from: process.env.BREVO_FROM || process.env.SMTP_FROM || "Mediconeeds <no-reply@mediconeeds.com>",
  },
...
export const isBrevoConfigured = () => Boolean(config.brevo.apiKey);
