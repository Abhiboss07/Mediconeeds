...
export async function issueOtp({ identifier, channel, purpose }) {
  ...
  const masked = channel === "email" ? id.replace(/^(.{2}).*(@.*)$/, "$1***$2") : "***" + String(id).slice(-4);
  console.info(`[OTP] request received: ${masked} channel=${channel} purpose=${purpose}`);

  const code = String(crypto.randomInt(0, 1_000_000)).padStart(6, "0");
  await Otp.create({ identifier: id, channel, purpose, codeHash: hash(code), expiresAt: new Date(Date.now() + TTL * 1000) });
  console.info(`[OTP] code generated & stored (expires in ${Math.round(TTL / 60)}m) — calling ${channel} provider…`);
  ...
  if (!res.ok) {
    // Never silently fail — log the real provider error for diagnosis.
    console.error(`[OTP] ${channel} delivery failed (provider=${res.provider}):`, res.error);
    const msg = channel === "email"
      ? "We couldn't send the email code right now. Please check the address or try SMS."
      : "We couldn't send the SMS code right now. Please try email instead.";
    return { ok: false, error: msg };
  }
  console.info(`[OTP] ${channel} delivery accepted by provider (${res.provider})`);
  return { ok: true };
}
