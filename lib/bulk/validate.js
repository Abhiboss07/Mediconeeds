...
export function validateRows(rows, opts = {}) {
  const known = new Set((opts.knownCategories || []).map((c) => String(c).toLowerCase().trim()));
  ...
  return rows.map((r) => {
    ...
    const category = safeText(d.category);
    ...
    else if (Number.isNaN(price)) errors.push("Price is not a valid number");
    else if (price < 0) errors.push("Price cannot be negative");
    if (mrp !== null && Number.isNaN(mrp)) errors.push("MRP is not a valid number");
    if (typeof price === "number" && !Number.isNaN(price) && typeof mrp === "number" && !Number.isNaN(mrp) && mrp > 0 && mrp < price)
      errors.push("MRP is less than Price");
    ...
    if (stock !== null && !Number.isNaN(stock)) {
      ...
      else stockVal = Math.floor(stock);
    }
    ...
    let gstVal = 18;
    if (gst !== null) {
      if (Number.isNaN(gst) || gst < 0 || gst > 28) errors.push("Invalid GST (must be 0-28%)");
      else { gstVal = gst; if (!GST_SLABS.includes(gst)) warnings.push("GST is not a standard slab (0/5/12/18/28)"); }
    }

    // --- category known? ---
    if (category && known.size && !known.has(category.toLowerCase())) warnings.push(`Unknown category "${category}"`);
